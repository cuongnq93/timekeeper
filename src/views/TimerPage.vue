...
const signalTimestamps = ref<Array<{ cardIndex: number; timestamp: number }>>([])
...
// Track signal timestamps for manual mode
watch(currentSignalIndex, (newIndex, oldIndex) => {
  if (manualMode.value && newIndex !== null && newIndex !== oldIndex && showSignal.value) {
    // Record the timestamp when a signal is shown in manual mode
    signalTimestamps.value.push({
      cardIndex: newIndex,
...
const startTimer = () => {
  ...
  timerInterval = setInterval(() => {
    currentTime.value++
    checkSignals()
    // Timer continues running past totalTime to track overtime
    // User must manually press END button to stop
  }, intervalMs)
}
...
const endSession = () => {
  ...
  const sessionData = {
    ...
    finalTime: currentTime.value,
    manualMode: manualMode.value,
    signalTimestamps: signalTimestamps.value,
    cards: config.value.cards
  }
    ...
    <header class="bg-gray-800 border-b border-gray-700 p-4">
      <div class="container mx-auto flex justify-between items-center">
        <div class="flex items-center gap-3">
          <h1 class="text-2xl font-bold">TimeKeeper</h1>
          <span
        ...
        <div class="flex gap-4 items-center">
          <!-- Toggle Switch -->
          <div class="flex items-center gap-3">
            <span class="text-sm font-medium text-gray-400">Auto</span>
            <button
              @click="manualMode = !manualMode"
              class="relative inline-flex h-7 w-14 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800"
              :class="manualMode ? 'bg-blue-600' : 'bg-gray-600'"
              role="switch"
              :aria-checked="manualMode"
            >
              <span
                class="inline-block h-5 w-5 transform rounded-full bg-white shadow-lg transition-transform"
                :class="manualMode ? 'translate-x-8' : 'translate-x-1'"
              ></span>
            </button>
            <span class="text-sm font-medium" :class="manualMode ? 'text-blue-400' : 'text-gray-400'">Manual</span>
          </div>
    ...
    <footer class="bg-gray-800 border-t border-gray-700 p-4">
      <div class="container mx-auto text-center text-sm text-gray-400">
        <div v-if="manualMode" class="space-y-1">
          <p><strong>Manual Mode:</strong> Space = Toggle Signal | ← → = Navigate Signals | E = End Session | R = Reset | S = Settings | M = Switch Mode</p>
        </div>
        <div v-else class="space-y-1">
          <p><strong>Auto Mode:</strong> Space = Start/Pause | E = End Session | R = Reset | S = Settings | M = Switch Mode</p>
